...
void redraw(Window* win, SDL_Texture* texture) {
    ...
    if (IMAGE_HEIGHT > WINDOW_HEIGHT || IMAGE_WIDTH > WINDOW_WIDTH) {
        ...
        if (imageAspectRatio > windowAspectRatio) {
            ...
            SDL_RenderCopy(win->renderer, texture, 0, &windowDestination);
...
int main(int argc, char * argv[]) {
    ...
    //try to improve zoom quality by inproving sampling technique
...
* Navigate forward/backwards in current directrory
* Animated GIFs
